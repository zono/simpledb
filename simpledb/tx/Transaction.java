 ...
 * ensureing that all transactions are serializable, recoverable,
...
public class Transaction {
   ...
   * Thoese objects are created during system initialization.
   ...
   * uncommited transaction. Finally
   * write a quescent checkpoint record to the log.
   * This method is called during systemstartup,
   * before user transaction begin.
   */
  public void recover() {
    bm.flushAll(txnum);
    recoveryMgr.recover();
   ...
   * then it calls the buffer to a retrieve the value.
   * 
   ...
   * specified offset of the specified block.
   * The method first obtain an SLock on the block,
   ...
   * passing in the LSM of the log record and the transaction's id.
   * 
   * @param blk    a reference to the disk block
   * @param offset a byte offset within that block
   * @param val    the value to be stored
   */
  public void setInt(BlockId blk, int offset, int val, boolean ofToLog) {
    concurMgr.xLock(blk);
    Buffer buff = mybuffers.getBuffer(blk);
    int lsn = -1;
    if (ofToLog)
   ...
   * 
   * @param blk    a reference to the disk block
   * @param offset a byte offset within that blcok
   * @param val    the value to be stored
   */
  public void setString(BlockId blk, int offset, String val, boolean okToLong) {
    concurMgr.xLock(blk);
    Buffer buff = mybuffers.getBuffer(blk);
    int lsn = -1;
    if (okToLong)
  ...
  /**
   * Returen the number of blocks in the specified file.
