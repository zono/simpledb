 ...
 */
public class IndexUpdatePlanner implements UpdataPlanner {
  ...
  public int executeInsert(InsertData data, Transaction tx) {
    ...
    UpdataScan s = (UpdateScan) p.open();
    s.insert();
    RID rid = s.getRid();
  ...
  public int executeModify(ModifyData data, Transaction tx) {
    ...
    Index idex = (ii == null) ? null : ii.open();

    UpdataScan s = (UpdateScan) p.open();
    int count = 0;
    while (s.next()) {
      // first, update the record
      Constant newval = data.newValues().evaluate(s);
      ...
      if (idex != null) {
        RID rid = s.getRid();
        idex.delete(oldval, rid);
        idex.insert(newval, rid);
      }
      count++;
    }
    if (idx != null)
